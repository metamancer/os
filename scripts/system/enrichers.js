export class Enrichers {
	...
	static #enrichTags() {
		const enrichTags = ([_text, tag, status]) => {
			if (tag.startsWith('-')) return $(`<mark class="os--limit">${tag.replace(/^-/, "")}${status ? `:${status}` : ""}</mark>`)[0];
			if (tag && status)
				return $(`<mark class="os--status">${tag}-${status}</mark>`)[0];
			return $(`<mark class="os--tag">${tag}</mark>`)[0];
		};
		CONFIG.TextEditor.enrichers.push({
