...
function createStatus(data) {
	...
	return {
		name: data.name || t("Os.other.unnamed"),
		type: "ActiveEffect",
		flags: {
			os: {
				type,
...
export async function importCharacter(data) {
	if (data.compatibility && !["os", "empty"].includes(data.compatibility))
		return ui.notifications.warn("Os.ui.warn-incompatible-data", {
			localize: true,
		});
	...
	const themeData = Object.entries(data)
		...
		.map(([_, theme]) => ({
			name:
				...
				t("Os.other.unnamed", "TYPES.Item.theme"),
			type: "theme",
			system: {
	...
	if (created) {
		const formatted = game.i18n.format("Os.ui.info-imported-character", {
			name: created.name,
		});
