...
export async function importCharacter(data) {
	if (data.compatibility && !["os", "empty"].includes(data.compatibility))
		return ui.notifications.warn("Os.ui.warn-incompatible-data", { localize: true });
	...
	if (created) {
		const formatted = game.i18n.format("Os.ui.info-imported-character", { name: created.name });
