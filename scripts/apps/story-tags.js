...
export class StoryTagApp extends SheetMixin(FormApplication) {
	...
	get config() {
		const config = game.settings.get("os", "storytags");
		...
		return (
			this.config.actors
				...
				.map((actor) => ({
					...
					tags: actor.effects
						.filter((e) => !!e.flags.os?.type)
						.map((e) => ({
							id: e._id,
							name: e.name,
							values: e.flags.os.values,
							isBurnt: e.flags.os.isBurnt,
							value: e.flags.os.values.findLast((v) => !!v),
							type: e.flags.os.values.some((v) => !!v) ? "status" : "tag",
						}))
	...
	async setActors(actors) {
		await game.settings.set("os", "storytags", { ...this.config, actors });
		return this.#broadcastRender();
	}

	async setTags(tags) {
		await game.settings.set("os", "storytags", { ...this.config, tags });
		return this.#broadcastRender();
	}
	...
	activateListeners(html) {
		...
		game.socket.on("system.os", async (data) => {
		...
		this.#contextmenu = ContextMenu.create(
			...
			[
				{
					name: game.i18n.localize("Os.ui.remove-story-tags"),
				...
				{
					name: game.i18n.localize("Os.ui.remove-actors"),
			...
			{
				hookName: "OsStoryTagsContextMenu",
			},
		);
	...
	async _onDrop(dragEvent) {
		...
		if (
			...
			const tags = actor.system.tags.matchAll(CONFIG.os.tagStringRe);
	...
	async #addTagToActor({ id, tag }) {
		const actor = game.actors.get(id);
		if (!actor)
			return ui.notifications.error("Os.ui.error-no-actor", {
				localize: true,
			});
		if (!actor.isOwner)
			return ui.notifications.error("Os.ui.warn-not-owner", {
				localize: true,
			});
		...
		await actor.createEmbeddedDocuments("ActiveEffect", [
			{
				name: tag.name,
				flags: { os: { type: "tag", values: tag.values, isBurnt: false } },
			},
		]);
		return this.#broadcastRender();
	}
