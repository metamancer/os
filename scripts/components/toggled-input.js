export class ToggledInput extends HTMLElement {
	...
	static Register() {
		customElements.define("os-toggled-input", ToggledInput);
	}
	...
	async #renderTags(editMode = false) {
		switch (editMode) {
			case true:
				this.#renderedTags.classList.add("os--sro");
				this.#input.classList.remove("os--sro");
				break;
			default: {
				const tags = await TextEditor.enrichHTML(this.#input.value);
				this.#renderedTags.innerHTML = tags;
				this.#input.classList.add("os--sro");
				this.#renderedTags.classList.remove("os--sro");
			}
		}
	}
}
